class Page < ApplicationRecord
  ...
  def update_component(block, comp_data)
    comp_data.each_pair do |order, data|
      comp = data["id"] ? block.components.find(data["id"]) : block.components.new 
      comp[:order] = order
  ...
  def set_url
    unless url.present?
      update_attribute :url, Rails.application.routes.url_helpers.user_site_page_path(:locale => "en", :user_id => site.user_id, :site_id => site.id, :id => id)
    end
  end
end
